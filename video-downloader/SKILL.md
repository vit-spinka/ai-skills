...
Script at `~/pers-repos/ai-skills/video-downloader/met_download.py`. Supports search by name or direct URL.

```bash
python ~/pers-repos/ai-skills/video-downloader/met_download.py "Dialogues des Carmelites" [output-dir]
python ~/pers-repos/ai-skills/video-downloader/met_download.py https://ondemand.metopera.org/performance/detail/<id> [output-dir]
```

**Selection logic:** prefers video over audio; picks most recent when multiple versions exist.

**Search API:** `https://middleware.ondemand.metopera.org/client/search/<query>` (no auth needed)

**How it works:**
1. Searches the API and picks the best match
2. Opens the performance page in a visible Chromium window (log in if prompted)
3. Intercepts the `master.m3u8` request from the Brightcove player
4. Passes that URL to yt-dlp — downloads best quality (~1080p, 4–5 GiB per opera)
